...
Drop royalty-free MP3 files into the `music/` directory. They'll be auto-tagged by mood from filename keywords:

```
music/
  chill-lofi-beat.mp3      -> mood: chill
  epic-cinematic-trailer.mp3 -> mood: epic
  happy-ukulele-song.mp3   -> mood: happy
...
| `FACE_TRACKING` | `true` | OpenCV face detection for smart vertical crop |
| `FACE_TRACKING_SMOOTHING` | `0.15` | Smoothing factor for crop window movement |
| `CHUNK_MINUTES` | `20` | Chunk size for long videos (minutes) |
| `CHUNK_OVERLAP_SECONDS` | `60` | Overlap between chunks (seconds) |
...
| `VOICEOVER_ENGINE` | `kokoro` | `kokoro` (offline, recommended), `edge` (online), or `piper` (offline) |
| `KOKORO_VOICE` | `af_heart` | Kokoro voice name (28 English voices available) |
| `MUSIC_MODE` | `off` | `off`, `auto` (mood-based from music/ dir), or `file` (single track) |
| `MUSIC_MOOD` | empty | Override mood: chill, happy, dark, epic, sad, focus, comedy, action |
| `MUSIC_VOLUME` | `0.08` | Background music volume (0.0–1.0, keep low) |
...
music/                  # royalty-free background music (drop MP3s here)
pipeline/
  youtube_search.py     # Data API search by views
  transcript.py         # captions + Whisper fallback
  downloader.py         # yt-dlp download
  analyzer.py           # AI -> interesting segments (8-signal virality scoring)
  captions.py           # SRT + karaoke ASS captions
  editor.py             # FFmpeg vertical crop + burn captions + face tracking
  context_writer.py     # AI title/description/tags
  uploader.py           # Google OAuth upload
  voiceover.py          # TTS commentary (Kokoro / edge-tts / piper)
  state.py              # SQLite state: dedup, run history, review workflow
  ai.py                 # AI provider abstraction (Groq, Gemini, Ollama)
  music.py              # Mood-based background music selection
  analytics.py          # YouTube performance tracking + learning loop
  discovery.py          # Auto-pick trending low-competition topics
  thumbnail.py          # Custom thumbnail generation
```
